...
RSpec.describe_current do
  ...
  context 'when there are active processes' do
    ...
    it 'expect to have proper consumer group and details' do
      ...
      expect(topic_data[0][:lag_stored]).to eq(213_731_273)
      expect(topic_data[0][:lag_stored_d]).to eq(-3)
      expect(topic_data[0][:lag_hybrid]).to eq(213_731_273)
      ...
      expect(topic_data[0][:process][:process][:id]).to eq('shinra:1:1')
