...
## Unreleased
- **[Feature]** Aggregate the Health views per topic instead of per partition so they stay usable with many topics/partitions. The top level shows one summary row per topic (lag, max/avg lag, a "skewed" flag, LSO risk, paused partitions) with drill-down to the per-partition Overview/Lags/Offsets/Changes/Cluster Lags of a topic. Skew detection is configurable via `config.ui.health.lags.skew_threshold` and `config.ui.health.lags.skew_minimum` (#112).
