...
## Unreleased
- **[Feature]** Restructure the Health section around per-topic aggregation so it stays usable with many topics/partitions (the old all-topics per-partition views became unusable at e.g. 20 topics x 100 partitions). The top level now has just two aggregated views - "Topics" (the new default) and "Cluster Lags" - each showing one summary row per topic: partition count (with a "no data" flag), total lag and its trend, the biggest single-partition lag, the average lag and a "skewed" flag (so lag concentrated on one hot/stuck partition is visible even when the total looks fine), the worst LSO risk state and the paused partition count. Each topic drills down to a per-topic detail page whose tabs (Overview, Lags, Offsets, Changes, Cluster Lags) carry the previous per-partition views, now scoped to a single topic. The old top-level `health/overview|lags|offsets|changes` paths redirect to `health/topics`. The lag skew detection is configurable via `config.ui.health.lags.skew_threshold` (default `3`, how far above the average the biggest partition lag must be) and `config.ui.health.lags.skew_minimum` (default `100`, the smallest lag worth flagging so trivial imbalances are ignored) (#112).
