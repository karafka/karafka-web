...
## Unreleased
- [Feature] Add a **Publish message** capability to the Explorer (Pro): publish an arbitrary message to a topic directly from its Explorer page, with an optional key, target partition and headers. The payload can be typed in or uploaded as a file (for binary data), and sent either as raw bytes or as JSON that is validated and re-serialized to its canonical form. Gated by a new `publish?` policy (`config.ui.policies.messages`), enabled by default (#956).
