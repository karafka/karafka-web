...
## 1.0.0 (Unreleased)
- [Enhancement] Set an explicit `min.insync.replicas` (capped to their own replication factor) on the Web UI's own topics at install/reset/migrate time, unless the topic's own config already sets one. Previously these topics never set this config explicitly, so they silently inherited the cluster's broker-level default; on a cluster configured for higher durability (e.g. `min.insync.replicas: 2`) that made a topic created with `replication_factor: 1` unwritable under `acks: all`. Closes #348.
