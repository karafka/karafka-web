...
- [Maintenance] Stop the intermittent dashboard-spec `MissingConsumersMetricsError` by waiting for the produced state/metrics records to be readable (`wait_for_state_data(require_ui: false)`) before `MigrateStatesData` reads them back, so the migrate step no longer races broker propagation under parallel-worker load.
