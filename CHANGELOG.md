...
## 1.0.0 (Unreleased)
- [Fix] Stop the Health pages from showing a permanently stale `lag`/`lag_stored`/`lag_hybrid` for long-paused partitions. A paused partition's librdkafka connection stops updating its cached high watermark once fetching stops, so the self-reported lag freezes at pause time while the real lag keeps growing. The Web UI's dedicated processing consumer now tracks how long each partition has been continuously reported as paused and, for partitions paused at least `min_pause_duration` (default 60s), periodically (at most every `refresh_interval`, default 30s) issues a single batched `Karafka::Admin#read_partition_offsets` call across all eligible topics to fetch a fresh high watermark, combining it with the partition's own still-accurate self-reported `committed_offset`/`stored_offset` to compute a corrected lag. The correction is stored in the materialized consumers state (schema bumped to 1.5.0) and overlaid onto paused partitions by `Ui::Models::Health` at render time, so every Health view inherits the fix. Any failure (timeout, broker error) gracefully degrades by leaving the previous correction in place rather than reverting to the stale self-reported value. All three settings are configurable via `Karafka::Web.setup { |config| config.processing.paused_partitions_lag.* }`. Closes #562.
