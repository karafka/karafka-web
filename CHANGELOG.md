...
## 1.0.0 (Unreleased)
- [Enhancement] Link the topic, partition and offsets on the Errors detail view to the Explorer, so you can jump straight from an error to the corresponding topic/partition/message. The `topic` links to the topic in the Explorer, the `partition` links to the partition, and the producer dispatch-error `offset` is now linked as well (previously only `first_offset`/`last_offset`/`committed_offset` were, and only when valid). Also linked the per-partition offset range shown in the Search results metadata to the Explorer (#1179).
