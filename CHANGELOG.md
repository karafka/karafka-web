...
- [Enhancement] Add a generic keyword filtering (search) box (a Pro-only feature) to the data-heavy Web UI listings (consumers, controls, jobs, routing (both the list and the per-topic details), per-process subscriptions, cluster brokers, cluster replication, broker/topic configs, recurring tasks, the health views, and the Topics/Explorer/DLQ/Scheduled Messages topic lists), so a specific consumer/topic/job can be found without scrolling. Filtering is powered by a new `Lib::Filter` engine that mirrors the `Lib::Sorter` wiring (a per-controller `filterable_attributes` allow-list plus a `filter` helper) and supports nested structures via match-propagation: filtering the health views by a topic name keeps that topic with all of its partitions while hiding the consumer groups and topics that do not match. Routing and subscriptions are filtered non-destructively (matching a topic name or a consumer/subscription group name) so the live app routing is never mutated. On the flat record listings (consumers, controls, jobs, configs, recurring tasks) the box includes a field selector so you can choose which attribute to filter on (e.g. filter processes by their assigned topic), with a "Search" and an always-present "Reset" button. The per-topic routing details page now also presents its settings as a flat, sortable attribute/value table (sortable headers in OSS and Pro, filtering in Pro), and `Lib::Sorter` was hardened to skip non-mutable enumerable values (such as a `Range`) instead of crashing on them (#1073).
