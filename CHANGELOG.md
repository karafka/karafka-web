...
- [Enhancement] Add a generic keyword filtering (search) box to the data-heavy Web UI listings (consumers, controls, jobs, routing, cluster brokers, broker/topic configs, recurring tasks, the health views, and the Topics/Explorer/DLQ/Scheduled Messages topic lists), so a specific consumer/topic/job can be found without scrolling. Filtering is powered by a new `Lib::Filter` engine that mirrors the `Lib::Sorter` wiring (a per-controller `filterable_attributes` allow-list plus a `filter` helper) and supports nested structures via match-propagation: filtering the health views by a topic name keeps that topic with all of its partitions while hiding the consumer groups and topics that do not match. Routing is filtered non-destructively so the live app routing is never mutated (#1073).
