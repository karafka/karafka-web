...
## 1.0.0 (Unreleased)
- [Fix] Stop long, dotted/underscored Pro Explorer, DLQ, and Topics topic-tile names from overflowing or overlapping their tile. `topic-tile-text` now sets `overflow-wrap: anywhere` so unbreakable names wrap inside the tile instead of spilling out, and the tile link now carries a `title` attribute showing the full topic name on hover.
