...
- [Fix] Stop a misclick on a Pro-gated OSS sidebar item (Health, Explorer, Cron, Schedules, Dead, Topics) from fully navigating away to the standalone "Pro Feature" upsell page. Gated nav items are now non-navigating buttons that show the gating info inline in a small popover (name, one-line explanation, and a link to learn more) instead of an `<a href>` pointing at the real Pro-only route. Closes #1106.
