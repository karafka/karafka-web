...
- **[Breaking]** Migrate the Web UI reporting topic pause configuration to Karafka's namespaced per-topic `pause(timeout:, max_timeout:, with_exponential_backoff:)` DSL, replacing the flat `pause_timeout`/`pause_max_timeout`/`pause_with_exponential_backoff` topic setters.
