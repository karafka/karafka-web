...
- [Fix] HTML-escape the arguments interpolated into flash messages by `format_flash`. Flash messages are rendered raw so the `<strong>` markup survives, so an argument carrying markup was emitted as-is. The recurring tasks `trigger`/`enable`/`disable` actions echo a task id taken straight from the URL without checking that the task exists; every other caller passes an integer or a value it has already proven exists. Not exploitable on a default deploy - those routes are same-site POSTs and the UI's CSP sets no `'unsafe-inline'` - so this closes a markup-injection sink rather than a live XSS.
