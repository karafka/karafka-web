...
function isEditableFormVisible() {
  ...
  for (let form of forms) {
    // Skip the always-present search/filter box: it is visible on almost every listing page, so
    // treating it as "an editable form in progress" would permanently block live polling. Typing
    // into it is still respected via isFormActive() (focused control), so we only skip it here.
    if (form.classList.contains('filter-form')) {
      continue;
    }
...
// The search/filter box is present on almost every listing, so its mere presence must not block
// polling (see isEditableFormVisible). But once the user has typed/changed something in it without
// submitting, refreshing #content would wipe that pending input - so we treat it as "in progress"
// only while it is dirty (its current value differs from what was rendered/submitted).
function isFilterFormDirty() {
  const forms = document.querySelectorAll('form.filter-form');

  for (let form of forms) {
    // Text inputs: dirty when the value differs from the rendered (submitted) value
    const inputs = form.querySelectorAll('input[type="text"]');
    for (let input of inputs) {
      if (input.value !== input.defaultValue) { return true; }
    }

    // Field selector: dirty when a different option than the rendered one is selected
    const selects = form.querySelectorAll('select');
    for (let select of selects) {
      const changed = Array.from(select.options).some(o => o.selected !== o.defaultSelected);
...
function isPollingPossible(check_url = false){
  ...
  if (isFilterFormDirty()) { return false }
...
function setLivePollButton() {
  ...
  if (localStorage.karafkaLivePoll == "enabled") {
    selector.classList.remove("!text-base-content/70");
    selector.classList.remove("hover:!text-primary-content")
  } else {
    selector.classList.add("!text-base-content/70")
