        ...
        module Lib
          module Consumers
            ...
            module Commands
              ...
              #
              # Numeric fields are kept as raw strings (via `fetch`, so a missing key does not
              # raise) and their format is validated by the contracts, so a malformed or absent
              # value fails validation instead of being silently coerced (`"abc".to_i => 0`).
              module Normalizer
                class << self
                  ...
                  #   {Transform} converts it to milliseconds when the payload is built.
                  def pause(params)
                    {
