        ...
        module Lib
          module Explorer
            ...
            module Publishing
              ...
              #
              # The payload is produced as raw bytes (an uploaded file wins over textarea); we do
              # not serialize it - producing takes raw bytes and Karafka has no producer-side /
              # topic serializers to apply. Header parsing lives here too; the form contract reuses
              # the `headers?` predicate so validation and transformation agree what "valid" means.
              module Transform
                class << self
                  ...
                  #   over textarea), or nil for a tombstone (e.g. to delete a key on a compacted
                  #   topic)
                  def payload(data)
