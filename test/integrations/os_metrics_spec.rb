...
assert_operator(memory_size, :>, 0, "Memory size should be positive")
...
assert_operator(memory_usage, :<, memory_size, "Process memory should be less than total memory")
...
assert_equal(3, cpu_usage.length, "CPU usage should return 3 load averages")
cpu_usage.each_with_index do |load, i|
  assert_type(load, Float, "Load average #{i} should be a float")
  assert_operator(load, :>=, 0, "Load average #{i} should be non-negative")
end
...
if memory_threads_ps
  ...
  assert_predicate(memory_threads_ps, :any?, "memory_threads_ps should not be empty")

  sample = memory_threads_ps.first

  assert_kind_of(Array, sample, "Each entry should be an array")
  assert_equal(3, sample.length, "Each entry should have 3 elements")
  ...
  assert_operator(memory_total, :>=, memory_usage, "Total memory should be >= process memory")

  threads = os_metrics.threads(memory_threads_ps)

  assert_operator(threads, :>=, 0, "Thread count should be non-negative")
