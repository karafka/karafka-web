...
describe_current do
  ...
  describe "#explorer_metadata_path" do
    # Minimal stand-in for a Karafka message exposing the coordinate readers the helper uses
    let(:msg) { Struct.new(:topic, :partition).new("default", 3) }

    context "when the key is :topic" do
      it "links to the topic in the explorer" do
        result = helper.explorer_metadata_path(:topic, "default", msg)

        assert_equal("/web-ui/explorer/topics/default", result)
      end

      it "returns nil when the value is nil" do
        assert_nil(helper.explorer_metadata_path(:topic, nil, msg))
      end
    end

    context "when the key is :partition" do
      it "links to the partition in the explorer" do
        result = helper.explorer_metadata_path(:partition, 3, msg)

        assert_equal("/web-ui/explorer/topics/default/3", result)
      end

      it "links even when the partition is 0" do
        result = helper.explorer_metadata_path(:partition, 0, msg)

        assert_equal("/web-ui/explorer/topics/default/0", result)
      end

      it "returns nil when the value is nil" do
        assert_nil(helper.explorer_metadata_path(:partition, nil, msg))
      end

      it "returns nil when the message has no topic" do
        msg.topic = nil

        assert_nil(helper.explorer_metadata_path(:partition, 3, msg))
      end
    end

    context "when the key is :offset" do
      it "links to the message in the explorer" do
        result = helper.explorer_metadata_path(:offset, 1, msg)

        assert_equal("/web-ui/explorer/topics/default/3/1", result)
      end

      it "links even when the offset is 0" do
        result = helper.explorer_metadata_path(:offset, 0, msg)

        assert_equal("/web-ui/explorer/topics/default/3/0", result)
      end

      it "returns nil when the value is nil" do
        assert_nil(helper.explorer_metadata_path(:offset, nil, msg))
      end

      it "returns nil when the message has no topic" do
        msg.topic = nil

        assert_nil(helper.explorer_metadata_path(:offset, 1, msg))
      end

      it "returns nil when the message has no partition" do
        msg.partition = nil
    ...
    context "when the key is not a linkable coordinate" do
      it "returns nil" do
        assert_nil(helper.explorer_metadata_path(:consumer, "SomeConsumer", msg))
      end
    end
  end
