...
describe_current do
  ...
  Dir[Karafka::Web.gem_root.join("lib", "karafka", "web", "pro", "**/*.*")].each do |pro_file|
    context "when checking #{pro_file}" do
      ...
      it { assert_includes(content, header_title) }
      it { assert_includes(content, header_license) }
    end
  end

  pro_path = Karafka::Web.gem_root.join("test", "lib", "karafka", "web", "pro", "**/*.*")
  ...
  # Validates that all test files have timing data for parallel test balancing.
  # If this fails, run: bin/collect_timings
  describe "test timings" do
    let(:timings_dir) { Karafka::Web.gem_root.join("test", "timings") }
