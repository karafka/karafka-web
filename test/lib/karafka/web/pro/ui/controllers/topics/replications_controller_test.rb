...
describe_current do
  ...
  describe "#show" do
    ...
    context "when the cluster has multiple brokers with an out-of-sync replica (simulated)" do
      # leader 1, replicas across brokers 1/2/3, broker 3 fallen out of the ISR set
      let(:partitions_data) do
        [
          {
            partition_id: 0, leader: 1, replica_count: 3, in_sync_replica_brokers: 2,
      ...
      before do
        ...
        Karafka::Admin.stubs(:read_watermark_offsets).returns([0, 0])
        Karafka::Admin.stubs(:read_watermark_offsets).with(errors_topic => [0]).returns({ errors_topic => { 0 => [0, 0] } })
        Karafka::Admin.stubs(:read_watermark_offsets).with(topic => [0]).returns({ topic => { 0 => [0, 100] } })
        get "topics/#{topic}/replication"
      end

      it "emphasizes the leader and highlights the out-of-sync replica" do
        assert_ok
        assert_body('<span class="badge badge-primary">1</span>') # leader
        assert_body('<span class="badge badge-info">2</span>')    # in-sync replica
        assert_body('<span class="badge badge-warning">3</span>') # out-of-sync replica
      end

      it "links each broker badge to its own broker details page" do
        assert_ok
        assert_body('title="Broker 1 details"')
        assert_body('title="Broker 2 details"')
        assert_body('title="Broker 3 details"')
        assert_body("cluster/2\"")
        assert_body("cluster/3\"")
      end
    end
  end
end
