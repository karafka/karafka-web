  ...
  describe "#extract_error_info" do
    context "with standard error scenarios" do
      it "extracts basic error information" do
        ...
        assert_equal("StandardError", result[0])
        assert_equal("test message", result[1])
      ...
      it "handles nil error message gracefully" do
        ...
        assert_equal("StandardError", result[0])
        assert_equal("StandardError", result[1])
        assert_kind_of(String, result[2])
      end
    ...
    context "with custom error classes" do
      it "extracts custom error class names" do
        ...
        assert_equal("TestError", result[0])
        assert_equal("custom error message", result[1])
      end
      ...
      it "handles nested error classes" do
        ...
        assert_equal("TestModule::NestedError", result[0])
        assert_equal("nested error", result[1])
      end
    end
    ...
    context "with backtrace processing" do
      ...
      it "processes backtrace and removes app root paths" do
        ...
        assert_includes(processed_backtrace, "app/models/user.rb:10:in `method1'")
        assert_includes(processed_backtrace, "/other/path/file.rb:20:in `method2'")
        assert_includes(processed_backtrace, "lib/helper.rb:30:in `method3'")
        refute_includes(processed_backtrace, app_root)
      end
      ...
      it "processes backtrace and removes gem home paths" do
        ...
        assert_includes(processed_backtrace, "gems/some_gem/lib/file.rb:10:in `gem_method'")
        assert_includes(processed_backtrace, "/other/path/file.rb:20:in `method2'")
        refute_includes(processed_backtrace, gem_home)
      end
      ...
      it "expect to return backtrace without the gem root path" do
        ...
        refute_includes(result[2], Karafka.gem_root.to_s)
      end
    end
    ...
    context "with complex backtrace scenarios" do
      ...
      it "handles backtrace with special characters" do
        ...
        assert_includes(backtrace_result, "with spaces")
        assert_includes(backtrace_result, "with-dashes")
        assert_includes(backtrace_result, "with.dots")
      end
    end
  end
  ...
  describe "#extract_error_message" do
    context "with standard message scenarios" do
      it "extracts simple error message" do
        ...
        assert_equal("simple message", result)
      end
    ...
    context "with message length limits" do
      ...
      it "preserves messages under limit" do
        ...
        assert_equal(5_000, result.length)
      ...
      it "expect to trim extremely long message to 10k of characters" do
        ...
        assert_equal(msg[0, 10_000], extracted_message)
      end
    end
